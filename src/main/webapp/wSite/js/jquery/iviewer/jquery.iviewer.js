...
$.widget( "ui.iviewer", $.ui.mouse, {
    ...
    **/
    ...
    rotate: function(type)
    {
    	...
    	//this.img_object.object.rotate({animateTo:this.angle});
    	this.img_object.object.rotate(this.angle);
    },
    ...
    **/
    ...
    {
        var me=this;
        var btnBaseClass = "iviewer_common iviewer_button";

        me.toolbar = $("<div>").addClass("ui-widget-header ui-corner-all iviewer_toolbar").
        css({position:"absolute",top:"0px", left:"0px"}).appendTo(this.container);
        me.toolbar.draggable({ containment: "parent", scroll: false });

        $("<div>").addClass(btnBaseClass).addClass("iviewer_zoom_in").attr("title","放大").
        mousedown(function(){me.zoom_by(1); return false;}).appendTo(this.toolbar);

        $("<div>").addClass(btnBaseClass).addClass("iviewer_zoom_out").attr("title","縮小").
        mousedown(function(){me.zoom_by(- 1); return false;}).appendTo(this.toolbar);

        $("<div>").addClass(btnBaseClass).addClass("iviewer_zoom_zero").attr("title","實際大小").
        mousedown(function(){me.set_zoom(100); return false;}).appendTo(this.toolbar);

        $("<div>").addClass(btnBaseClass).addClass("iviewer_zoom_fit").attr("title","適應圖像").
        mousedown(function(){me.fit(this); return false;}).appendTo(this.toolbar);
        
        $("<div>").addClass(btnBaseClass).addClass("iviewer_rotate_left").attr("title","向左翻轉").
        mousedown(function(){ me.rotate("left");return false;}).appendTo(this.toolbar);
        
        $("<div>").addClass(btnBaseClass).addClass("iviewer_rotate_right").attr("title","向右翻轉").
        mousedown(function(){ me.rotate("right");return false;}).appendTo(this.toolbar);

        this.zoom_object = $("<div>").addClass("iviewer_zoom_status").addClass("iviewer_common").
        appendTo(this.toolbar);
        
        $("<div>").addClass(btnBaseClass).addClass("iviewer_savefile").attr("title","儲存檔案").
        mousedown(function(){ 
        	if(me.options.onSaveFile)
        	{
        		if(me.angle==0 || me.angle==360) return false;
				me.options.onSaveFile(me.imgSrc,me.angle);
        	}
        	
        	return false; 
        }).appendTo(this.toolbar);
        
        $("<div>").addClass(btnBaseClass).addClass("iviewer_fullscreen").attr("title","Full Screen").
        mousedown(function(){ 
        	var imgUrl = me.imgSrc;
        	test(imgUrl);
        	//window.open("http://www.pchome.com.tw","xxx");
        	//window.open("photoViewer.jsp?img="+imgUrl,"檔案速覽視窗",'width='+window.screen.width+',height='+window.screen.height+',menubar=no,scrollbars=no,resizable=yes');
